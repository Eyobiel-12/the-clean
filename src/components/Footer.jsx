import { motion } from 'framer-motion'

export default function Footer() {
  const WHATSAPP_URL = `https://wa.me/31854015937?text=${encodeURIComponent('Hallo! Ik wil graag een afspraak plannen voor jullie schoonmaakdiensten.')}`

  return (
    <footer className="bg-stone-900 text-stone-100 py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        <div className="flex flex-col gap-10 sm:gap-12">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-8">
            <motion.div
              initial={{ opacity: 0 }}
              whileInView={{ opacity: 1 }}
              viewport={{ once: true }}
            >
              <img src="/logo.jpeg" alt="The Clean" className="h-10 sm:h-12 w-auto object-contain" />
            </motion.div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 sm:gap-16">
              <nav className="flex flex-wrap gap-6 sm:gap-12 text-sm">
                <a href="#gordijnen" className="hover:text-teal-400 transition-colors">Gordijnen</a>
                <a href="#banken" className="hover:text-teal-400 transition-colors">Banken</a>
                <a href="#over-ons" className="hover:text-teal-400 transition-colors">Over Ons</a>
                <a href="#diensten" className="hover:text-teal-400 transition-colors">Diensten</a>
                <a href="#galerij" className="hover:text-teal-400 transition-colors">Galerij</a>
                <a href="#contact" className="hover:text-teal-400 transition-colors">Contact</a>
              </nav>
              <address className="not-italic text-sm text-stone-400 space-y-1">
                <p className="text-stone-100 font-medium">Marktplein 36</p>
                <p>7311 LR Apeldoorn</p>
                <a href="tel:+31854015937" className="block mt-2 text-teal-400 hover:text-teal-300 transition-colors">+31 85 401 5937</a>
                <a href="mailto:info@alpgr.com" className="block text-teal-400 hover:text-teal-300 transition-colors">info@alpgr.com</a>
                <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-3 text-[#25D366] hover:text-[#34e077] transition-colors">
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                  </svg>
                  WhatsApp
                </a>
              </address>
            </div>
          </div>
        </div>
        <div className="mt-10 sm:mt-12 pt-8 border-t border-stone-700 text-center text-sm text-stone-500 space-y-2">
          <p>
            Onderdeel van{' '}
            <a href="https://www.alpha-groups.nl/" target="_blank" rel="noopener noreferrer" className="text-teal-400 hover:text-teal-300 transition-colors">Alpha Group</a>
          </p>
          <p>© {new Date().getFullYear()} The Clean. Professionele schoonmaakdiensten.</p>
        </div>
      </div>
    </footer>
  )
}
